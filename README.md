...
### **✨4차 과제 업데이트 내용:**
  * 내용.
  * 이중 리스트 기반 데이터 관리 : 단일 기록 저장이 아닌, 여러 플레이어의 이름, 주행 거리, 코인, 등급 정보를 이중 리스트 구조로 저장하고 append()를 통해 누적함.
  * 중첩 순회 리더보드 : 게임 종료 시 이중 리스트를 중첩 for문과 인덱싱으로 순회하여 전체 유저의 기록을 표 형식으로 깔끔하게 출력함.
  * 데이터 파일 영구 저장 : with open()과 write()를 사용하여 프로그램 종료 시 누적된 이중 리스트 데이터를 leaderboard.txt 파일에 자동으로 저장함.
  * 2중 예외 처리 : 사용자 입력 시 발생하는 ValueError와 파일 저장/로드 시의 오류를 방지하기 위해 최소 2곳 이상의 예외 처리 구문을 적용함.
    
### **🤖 AI 파트너십 과정**
 1. **내용 1** 이중 리스트와 파일 저장 로직 설계
    * **프롬프트 요약:**  파이썬 3D 게임에서 여러 명의 유저 성적을 이중 리스트로 관리하고, 종료 시 텍스트 파일로 저장하는 효율적인 함수 구조를 알려줘
    * **적용 내용:** AI가 제안한 save_results() 함수와 리스트 데이터를 문자열로 변화낳여 파일에 쓰는 로직을 실제 코드에 반영함
   
2. 내용 : 견고한 시스템을 위한 예외 처리 전략
   프롬프트 요약 : 사용자가 메뉴에 문자를 잘못 입력하거나 데이터 파일이 없을 때 프로그램이 멈추지 않게 하려면 try-except를 어디에 써야할까?
   적용 내용 : 입력부의 ValueError 대응과 파일 오픈 시의 안정성을 확보하기 위해 제안받은 위치에 예외 처리 코드를 배치함.
    
### **🛠️ Troubleshooting & 기술 회고:**
  1. **문제 1:** 파일 저장 시 데이터 형식 오류
     * **원인:** 이중 리스트 내의 숫자 데이터(거리, 코인)를 문자열 변환 없이 파일에 바로 쓰려고 시도하여 오류가 발생함.
     * **해결:** str() 함수를 사용하여 각 데이터를 문자열로 형변환하고 구분자(,)를 추가하여 저장하는 방식으로 수정함.
    
   2.문제2 : 이중 리스트 출력 시 정렬 깨짐 현상
    원인 : 각 데이터의 길이에 따라 리더보드 출력 결과의 세로 칸이 맞지 않아 가독성이 떨어짐.
    해결 : f-string의 정렬 기능을 사용하여 표 형식의 일정한 간격 출력을 구현함.
     
### **📁 증빙 자료:**
  * [4차_AI협업캡처.pdf 첨부 완료] (첨부 후 링크)
  * [4차과제_실행결과.jpg]
<br><img width="1812" height="2176" alt="KakaoTalk_20260601_095322796" src="https://github.com/user-attachments/assets/33866ccb-ad23-4af0-94b8-92c22c71ecf4" />
<img width="1812" height="2176" alt="KakaoTalk_20260601_095322796" src="https://github.com/user-attachments/assets/34e705a5-69a6-4865-a2ca-329f8a1756f2" />
<img width="1812" height="2176" alt="KakaoTalk_20260601_095322796" src="https://github.com/user-attachments/assets/a2761c69-6cce-4304-bafe-9a425683c32f" />
<img width="1812" height="2176" alt="KakaoTalk_20260601_095322796" src="https://github.com/user-attachments/assets/badb48cf-6f4b-4085-aa82-853c5c81a490" />
<img width="1812" height="2176" alt="KakaoTalk_20260601_095322796" src="https://github.com/user-attachments/assets/2e2552e3-00da-4d7e-8393-ce743be3da5c" />
